...
CREATE PROCEDURE insertarEmpleado(	/* Datos Personales */
                                    ...
                                    OUT var_lastToken       VARCHAR(65)     -- 23
				)                                    
    BEGIN        
        ...
        INSERT INTO usuario ( nombre, contrasenia, rol) 
                    VALUES( var_nombreUsuario, SHA2(var_contrasenia, 256), var_rol);
...
DELIMITER $$
CREATE PROCEDURE buscarEmpleado(IN var_letra VARCHAR(45))
SELECT * FROM optiqalumnos.v_empleados where nombre like  CONCAT('%',var_letra,'%');
$$
DELIMITER ;
...
UPDATE usuario SET lastToken = var_lastToken, dateLastToken = CURRENT_TIMESTAMP 
WHERE idUsuario = var_idUsuario;
$$
DELIMITER ;
